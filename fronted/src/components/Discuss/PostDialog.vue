<template>
    <el-dialog :visible.sync="dialogVisible" title="发布新帖子" width="50%">
      <el-form ref="postForm" :model="postForm" label-width="100px">
        ...
        <el-form-item label="内容" required :rules="[{ required: true, message: '请输入内容', trigger: 'blur' }]">
          <el-input type="textarea" v-model="postForm.content" rows="5"></el-input>
        </el-form-item>
        <el-form-item label="板块" required :rules="[{ required: true, message: '请选择板块', trigger: 'change' }]">
  ...
  export default {
    ...
    methods: {
      ...
      resetForm() {
        ...
        this.dialogVisible = false; // 重置后关闭对话框
      }
    }
  }
  </script>
