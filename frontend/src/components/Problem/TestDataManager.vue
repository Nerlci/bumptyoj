...
export default {
  ...
  methods: {
    fetchData() {
      axios
        .get(`/api/problem/testdata?problemId=${this.problemId}`)
        .then((response) => {
    ...
    editData(data) {
      ...
      axios
        .post(`/api/problem/testdata?problemId=${this.problemId}`, formData)
        .then((response) => {
          if (response.data.code === "200") {
    ...
    deleteData(testdataId) {
      axios
        .delete(`/api/problem/testdata?testDataId=${testdataId}`)
        .then((response) => {
          if (response.data.code === "200") {
