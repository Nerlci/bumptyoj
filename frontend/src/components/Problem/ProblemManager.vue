<template>
  <div class="problem-manager" v-loading="loading">
    ...
    >
      ...
      <el-form-item>
        <el-button @click="operation === '添加' ? add : update" type="primary"
          >{{ operation }}题目</el-button
        >
        <!-- 根据 operation 显示删除按钮 -->
        <el-button
          v-if="operation === '更新'"
  ...
  methods: {
    create() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          ...
          this.postRequest("/api/problem/problem", requestData).then(
            (response) => {
    ...
    update() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          ...
          this.putRequest(
            `/api/problem/problem?problemId=${this.problem.problemId}`,
            requestData,
          ).then((response) => {
            if (response.code === "200") {
              this.$message({
              ...
              this.$router.push({
                name: "problemDetail",
                params: { id: this.problem.problemId },
              });
            } else {
    ...
    deleteProblem() {
      ...
      this.$confirm("确定要删除这个题目吗？操作不可恢复!", "警告", {
        ...
        .then(() => {
          this.deleteRequest(
            `/api/problem/problem?problemId=${this.problem.id}`,
          ).then((response) => {
            if (response.code === "200") {
              this.$message({
                type: "success",
